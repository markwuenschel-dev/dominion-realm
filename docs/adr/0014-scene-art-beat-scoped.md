...
- **Reading first, timeline now shipped.** The reader landed on `/read` first;
  ...
  calls `getSceneMediaMap` for one batched read of every beat's Scene art and
  renders each beat's Scene art inline
  inside its reveal gate (non-priority, below the beat summary) — not as a single
  top-of-page plate. Reading still uses single-item `getSceneMedia`. The reader
  filters on `beat`, so a chapter and an Event that
