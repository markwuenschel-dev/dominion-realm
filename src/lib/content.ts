...
/**
 * Relationship collection constraint. Keystatic may write the sentinel
 * `unset` (see RELATIONSHIP_COLLECTION_OPTIONS); that means "no constraint"
 * and must parse to `undefined` so a CMS-default save never fails the build
 * (audit CAND-01). Exported for the fitness test that pins write ⊆ read.
 */
