...
export function Button({ children, className, onClick }: { children: React.ReactNode; className?: string; onClick?: () => void }) {
  return <button onClick={onClick} className={cn('rounded px-3 py-1.5 text-sm transition-colors', className)}>{children}</button>
}

export function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input {...props} className={cn('rounded border border-rim bg-panel-raised px-2 py-1 text-sm text-foreground', props.className)} />
}
