...
function qToX(q: number) {
  return PAD.left + q * PLOT_W;
}
...
export function RegenCurveViz() {
  ...
  const { failurePath, safePath } = useMemo(() => {
    ...
    const toPathData = (pts: typeof curveSamples) => {
      ...
      const lines = rest
        .map((pt) => `L ${qToX(pt.q).toFixed(1)} ${mToY(pt.multiplier).toFixed(1)}`)
        .join(' ');
      return `${start} ${lines}`;
    };
    ...
    >
      <div className="overflow-x-auto">
        ...
        >
          ...
          {/* Axis labels */}
          <text x={PAD.left + PLOT_W / 2} y={H - 2} textAnchor="middle" fontSize={9} fill="#71717a">
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between">
        ...
        <span
          className={`stat-value text-sm font-semibold ${isDefault ? 'text-realm-gold-dim' : 'text-realm-gold'}`}
        >
          {format(value)}
        </span>
      </div>
